		...
		_init: function(b) {
			...
			if (b.code) {
				$("#verifyYz").click(function() {
					var phone=$("#phone").val();
					var _phone_v=/^1(3\d|5[0-35-9]|8[025-9]|47)\d{8}$/;
					if(_phone_v.test(phone)){//发送验证码
						...
						d._sendVerify();
						 $.ajax({
								...
								url : '/sso/user/isGetMechant?v=' + new Date().getTime(), //请求链接  
									...
									if(data.status!=1){
										//在这里进行ajax提交
										 $.ajax({
												...
												success : function(data) {
													if(data.status==1){
														$("#phone_v_error_show").text("短信已发送至您手机，请输入短信中的验证码");
													}else{
														$("#phone_v_error_show").text("请确认你的手机号码是否正确");
													}
												},
										...
										$("#phone_v_error_show").text("该用户已经被注册了，请重新输入手机号码");
										$("#phone").val("");
									}
								},
								error : function(XMLHttpRequest, data) {
