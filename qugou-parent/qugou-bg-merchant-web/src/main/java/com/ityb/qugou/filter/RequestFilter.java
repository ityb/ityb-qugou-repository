...
public class RequestFilter implements Filter{
	...
	public void doFilter(ServletRequest request,ServletResponse response,FilterChain chain) throws IOException,ServletException{
		...
		String token = CookieUtils.getCookieValue(httpServletRequest, RedisConstantKey.MERCHANT_SESSION_ID);
		User user=(User) cacheService.get(RedisConstantKey.MERCHANT_SESSION_ID + "_" + token);
		if(requestURI.endsWith("/save")||requestURI.endsWith("/saveDtoFile")||requestURI.endsWith("/add")){//表示为添加方法,需要拼接字符串
			if(user!=null){
				extraParams.put("creater", user.getId());
			}
		}
		if(requestURI.endsWith("/save-update")||requestURI.endsWith("/save-update-file")||requestURI.endsWith("/update")){//表示为添加方法,需要拼接字符串
			if(user!=null){
