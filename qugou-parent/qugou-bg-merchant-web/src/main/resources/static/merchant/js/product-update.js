...
layui.use(['layedit','upload','form'], function(){
	...
	//进行提交处理
		$("#product_submit").click(function(){
			...
			var picUrl='';
			$("#pic_show").find("img").each(function(index,value){
				picUrl+=$(value).attr("src")+","; 
			});
			picUrl+=$("#productPic").val();
		   ...
		   $.ajax({
				...
				dataType : "json", //数据传输格式  
				url : '/merchant/product/update', //请求链接  
				success : function(data) {
					if(data.status ==1) {
						layer.msg("添加成功");
		...
		$("#reselectSpecification").click(function(){
			$.ajax({
				async : true, //是否异步  
				cache : false, //是否使用缓存  
				type : 'post', //请求方式,post  
