...
layui.use(['paging', 'form','laydate'], function() {
    ...
    paging.init({
        openWait: true,
        url: 'http://www.qugou.com/merchant/product/list?v=' + new Date().getTime(), //地址
		elem: '#content', //内容容器
		params: { //发送到服务端的参数
		...
		complate: function() { //完成的回调
			...
			$('#content').children('tr').each(function() {
				...
				$that.children('td:last-child').children('a[data-opt=look]').on('click', function() {
					...
					$.get('/merchant/product/lookProduct?productId='+$(this).data('name'), null, function(form) {
				...
				$that.children('td:last-child').children('a[data-opt=del]').on('click', function() {
					...
					layer.confirm('确认要删除吗？', {
			            ...
			            $.ajax({
							...
							url : '/merchant/product/delete', //请求链接  
	...
	$('#up_product').on('click', function() {
		layer.confirm('确认要上架吗？', {
			...
			if(ids==''||ids==null){
				layer.msg('请选择需要上架的商品', {icon: 5,time:2000});
				return;
			}
			...
			$.ajax({
				...
				dataType : "json", //数据传输格式  
				url : '/merchant/product/updateProductState', //请求链接  
				success : function(data) {
					if(data.status ==1) {
						layer.msg("上架成功");
						location.reload(); //刷新
	...
	$('#down_product').on('click', function() {
		layer.confirm('确认要下架吗？', {
			...
			if(ids==''||ids==null){
				layer.msg('请选择需要下架的商品', {icon: 5,time:2000});
				return;
			}
			ids=ids.substring(0,ids.length-1);
			$.ajax({
